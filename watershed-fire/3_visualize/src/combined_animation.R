  ...
  ggplot() +
    ...
    theme(
      ...
      axis.text = element_text(
        color = "gray40", family = font_chart_axes,
        face = "plain", size = 5
      )
    )
}
...
combine_plots <- function(chart_data,
  ...
  plot_grid(plot_top, plot_bottom, ggdraw(), ncol = 1, rel_heights = c(1.8, 1, 0.26)) +
    ...
    draw_label(
      ...
      size = 5.5, color = "gray50", fontfamily = font_chart_titles, fontface = "bold"
    ) +

    # Add year
    draw_label(
      label = floor(year), x = 0.03, y = 0.43, hjust = 0,
      size = 18, color = "gray50", fontfamily = font_year, fontface = "bold"
    ) +
